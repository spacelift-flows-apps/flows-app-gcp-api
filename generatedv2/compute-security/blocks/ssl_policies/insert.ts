  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.custom_features !== undefined)
          body.custom_features = input.event.inputConfig.custom_features;
        if (input.event.inputConfig.description !== undefined)
          body.description = input.event.inputConfig.description;
        if (input.event.inputConfig.enabled_features !== undefined)
          body.enabled_features = input.event.inputConfig.enabled_features;
        ...
        if (input.event.inputConfig.min_tls_version !== undefined)
          body.min_tls_version = input.event.inputConfig.min_tls_version;
        if (input.event.inputConfig.name !== undefined)
          body.name = input.event.inputConfig.name;
        if (input.event.inputConfig.profile !== undefined)
          body.profile = input.event.inputConfig.profile;
        ...
        if (input.event.inputConfig.warnings !== undefined)
          body.warnings = input.event.inputConfig.warnings;

        const result = await computeFetch({
          config: input.app.config,
          method: "POST",
          pathTemplate: "/compute/v1/projects/{project}/global/sslPolicies",
          pathParams,
          queryParams,
