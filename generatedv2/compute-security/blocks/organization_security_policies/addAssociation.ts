  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.replace_existing_association !== undefined)
          queryParams["replaceExistingAssociation"] = String(
            input.event.inputConfig.replace_existing_association,
          );
        if (input.event.inputConfig.request_id !== undefined)
          queryParams["requestId"] = String(input.event.inputConfig.request_id);
        const body: Record<string, any> = {};
        if (input.event.inputConfig.attachment_id !== undefined)
          body.attachment_id = input.event.inputConfig.attachment_id;
        if (input.event.inputConfig.display_name !== undefined)
          body.display_name = input.event.inputConfig.display_name;
        if (input.event.inputConfig.excluded_folders !== undefined)
          body.excluded_folders = input.event.inputConfig.excluded_folders;
        if (input.event.inputConfig.excluded_projects !== undefined)
          body.excluded_projects = input.event.inputConfig.excluded_projects;
        if (input.event.inputConfig.name !== undefined)
          body.name = input.event.inputConfig.name;
        if (input.event.inputConfig.security_policy_id !== undefined)
          body.security_policy_id = input.event.inputConfig.security_policy_id;
