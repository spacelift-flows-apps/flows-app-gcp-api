...
const organizationSecurityPoliciesAddAssociation: AppBlock = {
  name: "Organization Security Policies - Add Association",
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.replaceExistingAssociation !== undefined)
          queryParams["replaceExistingAssociation"] = String(
            input.event.inputConfig.replaceExistingAssociation,
          );
        if (input.event.inputConfig.requestId !== undefined)
          queryParams["requestId"] = String(input.event.inputConfig.requestId);
        const body: Record<string, any> = {};
        if (input.event.inputConfig.attachmentId !== undefined)
          body.attachmentId = input.event.inputConfig.attachmentId;
        if (input.event.inputConfig.displayName !== undefined)
          body.displayName = input.event.inputConfig.displayName;
        if (input.event.inputConfig.excludedFolders !== undefined)
          body.excludedFolders = input.event.inputConfig.excludedFolders;
        if (input.event.inputConfig.excludedProjects !== undefined)
          body.excludedProjects = input.event.inputConfig.excludedProjects;
        if (input.event.inputConfig.name !== undefined)
          body.name = input.event.inputConfig.name;
        if (input.event.inputConfig.securityPolicyId !== undefined)
          body.securityPolicyId = input.event.inputConfig.securityPolicyId;
        ...
        const result = await computeFetch({
          ...
          method: "POST",
          pathTemplate:
            "/compute/v1/locations/global/securityPolicies/{security_policy}/addAssociation",
          pathParams,
          queryParams,
...
export default organizationSecurityPoliciesAddAssociation;
