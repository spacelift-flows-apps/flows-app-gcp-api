...
export async function getAlertPolicyServiceClient(
  config: Record<string, any>,
): Promise<any> {
  const credentials = await createCredentials(config);
  const Service = getService("google.monitoring.v3", "AlertPolicyService");
  return new Service("monitoring.googleapis.com:443", credentials);
}

export async function getGroupServiceClient(
  config: Record<string, any>,
): Promise<any> {
  const credentials = await createCredentials(config);
  const Service = getService("google.monitoring.v3", "GroupService");
  return new Service("monitoring.googleapis.com:443", credentials);
}

export async function getMetricServiceClient(
  config: Record<string, any>,
): Promise<any> {
  const credentials = await createCredentials(config);
  const Service = getService("google.monitoring.v3", "MetricService");
  return new Service("monitoring.googleapis.com:443", credentials);
}
...
export async function getUptimeCheckServiceClient(
  config: Record<string, any>,
): Promise<any> {
  const credentials = await createCredentials(config);
  const Service = getService("google.monitoring.v3", "UptimeCheckService");
  return new Service("monitoring.googleapis.com:443", credentials);
}
