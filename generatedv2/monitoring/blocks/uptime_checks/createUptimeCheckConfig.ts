...
const createUptimeCheckConfig: AppBlock = {
  ...
  inputs: {
    default: {
      config: {
        ...
        uptime_check_config: {
          name: "Uptime Check Config",
          description: "Required. The new Uptime check configuration.",
          type: {
            type: "object",
            properties: {
              name: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.uptime_check_config !== undefined)
          request.uptime_check_config =
            input.event.inputConfig.uptime_check_config;

        const result = await new Promise<any>((resolve, reject) => {
          client.createUptimeCheckConfig(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
        ...
        properties: {
          ...
          selected_regions: {
            ...
            items: {
              ...
              description: "The regions from which an Uptime check can be run.",
            },
            description:
              "The list of regions from which the check will be run. Some regions contain one location, and others contain more than one. If this field is specified, enough regions must be provided to include a minimum of 3 locations.  Not specifying this field will result in Uptime checks running from all available regions.",
          },
...
export default createUptimeCheckConfig;
