...
const updateCryptoKeyVersion: AppBlock = {
  name: "Update Crypto Key Version",
  description: `Update a [CryptoKeyVersion][google.cloud.kms.v1.CryptoKeyVersion]'s metadata. [state][google.cloud.kms.v1.CryptoKeyVersion.state] may be changed between [ENABLED][google.cloud.kms.v1.CryptoKeyVersion.CryptoKeyVersionState.ENABLED] and [DISABLED][google.cloud.kms.v1.CryptoKeyVersion.CryptoKeyVersionState.DISABLED] using this method. See [DestroyCryptoKeyVersion][google.cloud.kms.v1.KeyManagementService.DestroyCryptoKeyVersion] and [RestoreCryptoKeyVersion][google.cloud.kms.v1.KeyManagementService.RestoreCryptoKeyVersion] to move between other states.`,
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.crypto_key_version !== undefined)
          request.crypto_key_version =
            input.event.inputConfig.crypto_key_version;
        if (input.event.inputConfig.update_mask !== undefined)
          request.update_mask = input.event.inputConfig.update_mask;

        const result = await new Promise<any>((resolve, reject) => {
          client.updateCryptoKeyVersion(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
...
export default updateCryptoKeyVersion;
