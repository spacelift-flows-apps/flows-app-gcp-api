    ...
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.algorithm !== undefined)
          request.algorithm = input.event.inputConfig.algorithm;
        if (input.event.inputConfig.import_job !== undefined)
          request.import_job = input.event.inputConfig.import_job;
        if (input.event.inputConfig.wrapped_key !== undefined)
          request.wrapped_key = input.event.inputConfig.wrapped_key;
        if (input.event.inputConfig.rsa_aes_wrapped_key !== undefined)
          request.rsa_aes_wrapped_key =
            input.event.inputConfig.rsa_aes_wrapped_key;
