...
const updateCryptoKeyPrimaryVersion: AppBlock = {
  name: "Update Crypto Key Primary Version",
  description: `Update the version of a [CryptoKey][google.cloud.kms.v1.CryptoKey] that will be used in [Encrypt][google.cloud.kms.v1.KeyManagementService.Encrypt]. Returns an error if called on a key whose purpose is not [ENCRYPT_DECRYPT][google.cloud.kms.v1.CryptoKey.CryptoKeyPurpose.ENCRYPT_DECRYPT].`,
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.crypto_key_version_id !== undefined)
          request.crypto_key_version_id =
            input.event.inputConfig.crypto_key_version_id;
...
export default updateCryptoKeyPrimaryVersion;
