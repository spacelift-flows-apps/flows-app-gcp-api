...
const rollbackNodePoolUpgrade: AppBlock = {
  name: "Rollback Node Pool Upgrade",
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.respect_pdb !== undefined)
          request.respect_pdb = input.event.inputConfig.respect_pdb;

        const result = await new Promise<any>((resolve, reject) => {
          client.rollbackNodePoolUpgrade(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
...
export default rollbackNodePoolUpgrade;
