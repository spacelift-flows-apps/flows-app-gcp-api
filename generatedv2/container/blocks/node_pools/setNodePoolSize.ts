...
const setNodePoolSize: AppBlock = {
  name: "Set Node Pool Size",
  description: `Sets the size for a specific node pool. The new size will be used for all replicas, including future replicas created by modifying [NodePool.locations][google.container.v1.NodePool.locations].`,
  category: "Node Pools",
  inputs: {
    default: {
      config: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.node_count !== undefined)
          request.node_count = input.event.inputConfig.node_count;
        if (input.event.inputConfig.name !== undefined)
          request.name = input.event.inputConfig.name;

        const result = await new Promise<any>((resolve, reject) => {
          client.setNodePoolSize(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
...
export default setNodePoolSize;
