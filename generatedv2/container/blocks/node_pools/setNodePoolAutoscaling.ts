...
const setNodePoolAutoscaling: AppBlock = {
  name: "Set Node Pool Autoscaling",
    ...
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.autoscaling !== undefined)
          request.autoscaling = input.event.inputConfig.autoscaling;
        if (input.event.inputConfig.name !== undefined)
          request.name = input.event.inputConfig.name;

        const result = await new Promise<any>((resolve, reject) => {
          client.setNodePoolAutoscaling(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
...
export default setNodePoolAutoscaling;
