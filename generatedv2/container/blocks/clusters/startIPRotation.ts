...
const startIPRotation: AppBlock = {
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.rotate_credentials !== undefined)
          request.rotate_credentials =
            input.event.inputConfig.rotate_credentials;

        const result = await new Promise<any>((resolve, reject) => {
          client.startIPRotation(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
...
export default startIPRotation;
