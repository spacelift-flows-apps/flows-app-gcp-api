...
const setMonitoringService: AppBlock = {
  name: "Set Monitoring Service",
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.monitoring_service !== undefined)
          request.monitoring_service =
            input.event.inputConfig.monitoring_service;
        if (input.event.inputConfig.name !== undefined)
          request.name = input.event.inputConfig.name;

        const result = await new Promise<any>((resolve, reject) => {
          client.setMonitoringService(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
...
export default setMonitoringService;
