...
const setLoggingService: AppBlock = {
  name: "Set Logging Service",
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.logging_service !== undefined)
          request.logging_service = input.event.inputConfig.logging_service;
        if (input.event.inputConfig.name !== undefined)
          request.name = input.event.inputConfig.name;

        const result = await new Promise<any>((resolve, reject) => {
          client.setLoggingService(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
...
export default setLoggingService;
