...
const setNetworkPolicy: AppBlock = {
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.network_policy !== undefined)
          request.network_policy = input.event.inputConfig.network_policy;
        if (input.event.inputConfig.name !== undefined)
          request.name = input.event.inputConfig.name;

        const result = await new Promise<any>((resolve, reject) => {
          client.setNetworkPolicy(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
...
export default setNetworkPolicy;
