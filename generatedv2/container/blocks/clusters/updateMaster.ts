  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.master_version !== undefined)
          request.master_version = input.event.inputConfig.master_version;
        if (input.event.inputConfig.name !== undefined)
          request.name = input.event.inputConfig.name;

        const result = await new Promise<any>((resolve, reject) => {
          client.updateMaster(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
