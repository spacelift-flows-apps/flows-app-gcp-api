...
const setLegacyAbac: AppBlock = {
  name: "Set Legacy Abac",
    ...
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.enabled !== undefined)
          request.enabled = input.event.inputConfig.enabled;
        if (input.event.inputConfig.name !== undefined)
          request.name = input.event.inputConfig.name;

        const result = await new Promise<any>((resolve, reject) => {
          client.setLegacyAbac(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
