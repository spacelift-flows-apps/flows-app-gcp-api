...
const enableServiceAccountKey: AppBlock = {
  name: "Enable Service Account Key",
  description: `Enable a [ServiceAccountKey][google.iam.admin.v1.ServiceAccountKey].`,
    ...
    default: {
      ...
      onEvent: async (input) => {
        ...
        const result = await new Promise<any>((resolve, reject) => {
          client.enableServiceAccountKey(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
...
export default enableServiceAccountKey;
