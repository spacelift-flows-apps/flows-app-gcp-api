...
const targetInstancesInsert: AppBlock = {
  name: "Target Instances - Insert",
    ...
    default: {
      config: {
        ...
        natPolicy: {
          ...
          type: {
            type: "string",
            enum: ["UNDEFINED_NAT_POLICY", "NO_NAT"],
            description:
              "Must have a value of NO_NAT. Protocol forwarding delivers packets while preserving the destination IP address of the forwarding rule referencing the target instance. Check the NatPolicy enum for the list of possible values.",
          },
          required: false,
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.natPolicy !== undefined)
          body.natPolicy = input.event.inputConfig.natPolicy;
...
export default targetInstancesInsert;
