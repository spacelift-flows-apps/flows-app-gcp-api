...
const addSignedUrlKey: AppBlock = {
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.key_name !== undefined)
          body.key_name = input.event.inputConfig.key_name;
        if (input.event.inputConfig.key_value !== undefined)
          body.key_value = input.event.inputConfig.key_value;
...
export default addSignedUrlKey;
