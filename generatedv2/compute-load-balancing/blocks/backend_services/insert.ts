  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.affinity_cookie_ttl_sec !== undefined)
          body.affinity_cookie_ttl_sec =
            input.event.inputConfig.affinity_cookie_ttl_sec;
        ...
        if (input.event.inputConfig.circuit_breakers !== undefined)
          body.circuit_breakers = input.event.inputConfig.circuit_breakers;
        if (input.event.inputConfig.compression_mode !== undefined)
          body.compression_mode = input.event.inputConfig.compression_mode;
        if (input.event.inputConfig.connection_draining !== undefined)
          body.connection_draining =
            input.event.inputConfig.connection_draining;
        if (input.event.inputConfig.connection_tracking_policy !== undefined)
          body.connection_tracking_policy =
            input.event.inputConfig.connection_tracking_policy;
        if (input.event.inputConfig.consistent_hash !== undefined)
          body.consistent_hash = input.event.inputConfig.consistent_hash;
        ...
        if (input.event.inputConfig.custom_metrics !== undefined)
          body.custom_metrics = input.event.inputConfig.custom_metrics;
        if (input.event.inputConfig.custom_request_headers !== undefined)
          body.custom_request_headers =
            input.event.inputConfig.custom_request_headers;
        ...
        if (input.event.inputConfig.enable_c_d_n !== undefined)
          body.enable_c_d_n = input.event.inputConfig.enable_c_d_n;
        if (
          input.event.inputConfig.external_managed_migration_state !== undefined
        )
          body.external_managed_migration_state =
            input.event.inputConfig.external_managed_migration_state;
        if (
          input.event.inputConfig
            .external_managed_migration_testing_percentage !== undefined
        )
          body.external_managed_migration_testing_percentage =
            input.event.inputConfig.external_managed_migration_testing_percentage;
        if (input.event.inputConfig.failover_policy !== undefined)
          body.failover_policy = input.event.inputConfig.failover_policy;
        if (input.event.inputConfig.fingerprint !== undefined)
          body.fingerprint = input.event.inputConfig.fingerprint;
        if (input.event.inputConfig.ha_policy !== undefined)
          body.ha_policy = input.event.inputConfig.ha_policy;
        if (input.event.inputConfig.health_checks !== undefined)
          body.health_checks = input.event.inputConfig.health_checks;
        if (input.event.inputConfig.iap !== undefined)
          body.iap = input.event.inputConfig.iap;
        if (input.event.inputConfig.id !== undefined)
          body.id = input.event.inputConfig.id;
        if (input.event.inputConfig.ip_address_selection_policy !== undefined)
          body.ip_address_selection_policy =
            input.event.inputConfig.ip_address_selection_policy;
        ...
        if (input.event.inputConfig.locality_lb_policies !== undefined)
          body.locality_lb_policies =
            input.event.inputConfig.locality_lb_policies;
        if (input.event.inputConfig.locality_lb_policy !== undefined)
          body.locality_lb_policy = input.event.inputConfig.locality_lb_policy;
        if (input.event.inputConfig.log_config !== undefined)
          body.log_config = input.event.inputConfig.log_config;
        if (input.event.inputConfig.max_stream_duration !== undefined)
          body.max_stream_duration =
            input.event.inputConfig.max_stream_duration;
        ...
        if (
          input.event.inputConfig.network_pass_through_lb_traffic_policy !==
          undefined
        )
          body.network_pass_through_lb_traffic_policy =
            input.event.inputConfig.network_pass_through_lb_traffic_policy;
        if (input.event.inputConfig.outlier_detection !== undefined)
          body.outlier_detection = input.event.inputConfig.outlier_detection;
        ...
        if (input.event.inputConfig.security_policy !== undefined)
          body.security_policy = input.event.inputConfig.security_policy;
        if (input.event.inputConfig.security_settings !== undefined)
          body.security_settings = input.event.inputConfig.security_settings;
        if (input.event.inputConfig.self_link !== undefined)
          body.self_link = input.event.inputConfig.self_link;
        if (input.event.inputConfig.service_bindings !== undefined)
          body.service_bindings = input.event.inputConfig.service_bindings;
        if (input.event.inputConfig.service_lb_policy !== undefined)
          body.service_lb_policy = input.event.inputConfig.service_lb_policy;
        if (input.event.inputConfig.session_affinity !== undefined)
          body.session_affinity = input.event.inputConfig.session_affinity;
        if (
          input.event.inputConfig.strong_session_affinity_cookie !== undefined
        )
          body.strong_session_affinity_cookie =
            input.event.inputConfig.strong_session_affinity_cookie;
        ...
        if (input.event.inputConfig.tls_settings !== undefined)
          body.tls_settings = input.event.inputConfig.tls_settings;
        ...
        const result = await computeFetch({
          ...
          method: "POST",
          pathTemplate: "/compute/v1/projects/{project}/global/backendServices",
          pathParams,
          queryParams,
