...
const invalidateCache: AppBlock = {
  name: "Url Maps - Invalidate Cache",
    ...
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.url_map !== undefined)
          pathParams["url_map"] = String(input.event.inputConfig.url_map);
        ...
        if (input.event.inputConfig.cache_tags !== undefined)
          body.cache_tags = input.event.inputConfig.cache_tags;
        if (input.event.inputConfig.host !== undefined)
          body.host = input.event.inputConfig.host;
