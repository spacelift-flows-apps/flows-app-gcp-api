  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.failover !== undefined)
          request.failover = input.event.inputConfig.failover;

        const result = await new Promise<any>((resolve, reject) => {
          client.promoteReplica(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
