...
const pointInTimeRestore: AppBlock = {
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.context !== undefined)
          request.context = input.event.inputConfig.context;

        const result = await new Promise<any>((resolve, reject) => {
          client.pointInTimeRestore(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
