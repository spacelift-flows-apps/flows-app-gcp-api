  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.db_timeout !== undefined)
          request.db_timeout = input.event.inputConfig.db_timeout;

        const result = await new Promise<any>((resolve, reject) => {
          client.switchover(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
