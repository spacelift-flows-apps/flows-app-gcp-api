...
const startExternalSync: AppBlock = {
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.skip_verification !== undefined)
          request.skip_verification = input.event.inputConfig.skip_verification;
        ...
        if (input.event.inputConfig.replica_overwrite_enabled !== undefined)
          request.replica_overwrite_enabled =
            input.event.inputConfig.replica_overwrite_enabled;

        const result = await new Promise<any>((resolve, reject) => {
          client.startExternalSync(request, (err: any, response: any) => {
            if (err)
              reject(
                new Error(
...
export default startExternalSync;
