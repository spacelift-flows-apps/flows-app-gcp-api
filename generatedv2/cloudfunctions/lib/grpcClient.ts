...
export async function getFunctionServiceClient(
  config: Record<string, any>,
): Promise<any> {
  const credentials = await createCredentials(config);
  const Service = getService("google.cloud.functions.v2", "FunctionService");
  return new Service("cloudfunctions.googleapis.com:443", credentials);
}
