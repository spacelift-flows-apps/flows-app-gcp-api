...
import { getServicesClient, createRoutingMetadata, convertKeys } from "../../lib/grpcClient.ts";
  ...
  inputs: {
    default: {
        ...
        service: {
          ...
          type: {
                    ...
                    "properties": {
                              "name": {
                                        "type": "string",
                                        "description": "Identifier. The fully qualified name of this Service. In CreateServiceRequest, this field is ignored, and instead composed from CreateServiceRequest.parent and CreateServiceRequest.service_id.  Format: projects/{project}/locations/{location}/services/{service_id}"
                              },
                              "description": {
                                        "type": "string",
      ...
      onEvent: async (input) => {
        ...
        if (request.service?.name !== undefined) {
          const m = String(request.service?.name).match(/^([^/]+)/);
          if (m) routingParams["location"] = m[1];
        }
        const metadata = createRoutingMetadata(routingParams);
        const result = await new Promise<any>((resolve, reject) => {
          client.updateService(request, metadata, (err: any, response: any) => {
