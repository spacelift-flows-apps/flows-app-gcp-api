    ...
    default: {
      config: {
        ...
        source_instance_properties: {
          name: "Source Instance Properties",
          description:
        ...
        status: {
          name: "Status",
          description:
            "Output only. [Output Only] The status of the machine image. One of the following values:INVALID, CREATING, READY,DELETING, and UPLOADING. Check the Status enum for the list of possible values.",
          type: {
            type: "string",
            description:
              "Output only. [Output Only] The status of the machine image. One of the following values:INVALID, CREATING, READY,DELETING, and UPLOADING. Check the Status enum for the list of possible values.",
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.source_instance !== undefined)
          queryParams["sourceInstance"] = String(
            input.event.inputConfig.source_instance,
          );
        ...
        if (input.event.inputConfig.machine_image_encryption_key !== undefined)
          body.machine_image_encryption_key =
            input.event.inputConfig.machine_image_encryption_key;
        ...
        if (input.event.inputConfig.saved_disks !== undefined)
          body.saved_disks = input.event.inputConfig.saved_disks;
        if (input.event.inputConfig.self_link !== undefined)
          body.self_link = input.event.inputConfig.self_link;
        if (input.event.inputConfig.source_disk_encryption_keys !== undefined)
          body.source_disk_encryption_keys =
            input.event.inputConfig.source_disk_encryption_keys;
        if (input.event.inputConfig.source_instance !== undefined)
          body.source_instance = input.event.inputConfig.source_instance;
        if (input.event.inputConfig.source_instance_properties !== undefined)
          body.source_instance_properties =
            input.event.inputConfig.source_instance_properties;
        ...
        if (input.event.inputConfig.total_storage_bytes !== undefined)
          body.total_storage_bytes =
            input.event.inputConfig.total_storage_bytes;

        const result = await computeFetch({
          config: input.app.config,
          method: "POST",
          pathTemplate: "/compute/v1/projects/{project}/global/machineImages",
          pathParams,
          queryParams,
