  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.bigquery_config !== undefined)
          request.bigquery_config = input.event.inputConfig.bigquery_config;
        if (input.event.inputConfig.cloud_storage_config !== undefined)
          request.cloud_storage_config =
            input.event.inputConfig.cloud_storage_config;
        ...
        if (input.event.inputConfig.retain_acked_messages !== undefined)
          request.retain_acked_messages =
            input.event.inputConfig.retain_acked_messages;
        if (input.event.inputConfig.message_retention_duration !== undefined)
          request.message_retention_duration =
            input.event.inputConfig.message_retention_duration;
        if (input.event.inputConfig.labels !== undefined)
          request.labels = input.event.inputConfig.labels;
        if (input.event.inputConfig.enable_message_ordering !== undefined)
          request.enable_message_ordering =
            input.event.inputConfig.enable_message_ordering;
        if (input.event.inputConfig.expiration_policy !== undefined)
          request.expiration_policy = input.event.inputConfig.expiration_policy;
        if (input.event.inputConfig.filter !== undefined)
          request.filter = input.event.inputConfig.filter;
        if (input.event.inputConfig.dead_letter_policy !== undefined)
          request.dead_letter_policy =
            input.event.inputConfig.dead_letter_policy;
        if (input.event.inputConfig.retry_policy !== undefined)
          request.retry_policy = input.event.inputConfig.retry_policy;
        if (input.event.inputConfig.detached !== undefined)
          request.detached = input.event.inputConfig.detached;
        if (input.event.inputConfig.enable_exactly_once_delivery !== undefined)
          request.enable_exactly_once_delivery =
            input.event.inputConfig.enable_exactly_once_delivery;
        if (input.event.inputConfig.message_transforms !== undefined)
          request.message_transforms =
            input.event.inputConfig.message_transforms;
