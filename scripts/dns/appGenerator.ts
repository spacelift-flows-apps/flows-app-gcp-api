/**
 * App scaffolding generation for DNS REST apps.
 *
 ...
 */
function generateMainTs(config: DnsAppConfig): string {
...
3. Grant the **DNS Administrator** role (or more specific roles as needed)
...
This is used for all Cloud DNS API calls. You can find it in the
 ...
 */
function generateRestClient(baseUrl: string): string {
 ...
 */
function generateBlocksIndex(blocks: DnsGeneratedBlock[]): string {
  const importId = (b: DnsGeneratedBlock) =>
...
/**
 * Generate the package.json for a DNS app.
 */
 ...
 */
export async function writeDnsAppFiles(
  config: DnsAppConfig,
  blocks: DnsGeneratedBlock[],
