    ...
    default: {
      config: {
        ...
        updateMask: {
          ...
          type: {
            type: "string",
            description:
              "Required. Indicates which fields in the provided snapshot to update. Must be specified and non-empty. (Format: google-fieldmask)",
          },
          required: false,
        },
      },
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.snapshot !== undefined)
          requestBody.snapshot = input.event.inputConfig.snapshot;
