...
const nodePoolsRollback: AppBlock = {
  name: "Node Pools - Rollback",
  ...
  inputs: {
    default: {
      config: {
        respectPdb: {
          name: "Respect Pdb",
          description: "Option for rollback to ignore the PodDisruptionBudget.",
          type: {
            type: "boolean",
            description:
      ...
      onEvent: async (input) => {
        ...
        let path = `v1/projects/{projectId}/zones/{zone}/clusters/{clusterId}/nodePools/{nodePoolId}:rollback`;
        ...
        if (input.event.inputConfig.respectPdb !== undefined)
          requestBody.respectPdb = input.event.inputConfig.respectPdb;
...
export default nodePoolsRollback;
