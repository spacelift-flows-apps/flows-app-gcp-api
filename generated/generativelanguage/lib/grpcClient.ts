...
export async function getCacheServiceClient(
  config: Record<string, any>,
): Promise<any> {
  ...
  return new Service("generativelanguage.googleapis.com:443", credentials);
}

export async function getFileServiceClient(
  config: Record<string, any>,
): Promise<any> {
  const credentials = await createCredentials(config);
  const Service = getService(
...
export async function getGenerativeServiceClient(
  config: Record<string, any>,
): Promise<any> {
  const credentials = await createCredentials(config);
  const Service = getService(
...
export async function getPermissionServiceClient(
  config: Record<string, any>,
): Promise<any> {
  const credentials = await createCredentials(config);
  const Service = getService(
...
export async function getRetrieverServiceClient(
  config: Record<string, any>,
): Promise<any> {
  const credentials = await createCredentials(config);
  const Service = getService(
