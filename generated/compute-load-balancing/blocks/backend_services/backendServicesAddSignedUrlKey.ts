    ...
    default: {
      config: {
        ...
        keyValue: {
          name: "Key Value",
          description:
            "128-bit key value used for signing the URL. The key value must be a validRFC 4648 Section 5 base64url encoded string.",
          type: {
            type: "string",
            description:
              "128-bit key value used for signing the URL. The key value must be a validRFC 4648 Section 5 base64url encoded string.",
      ...
      onEvent: async (input) => {
        ...
        if (input.event.inputConfig.keyName !== undefined)
          body.keyName = input.event.inputConfig.keyName;
        if (input.event.inputConfig.keyValue !== undefined)
          body.keyValue = input.event.inputConfig.keyValue;
