...
const updateBuildTrigger: AppBlock = {
  name: "Update Build Trigger",
  ...
  inputs: {
    default: {
      config: {
        ...
        trigger: {
          name: "Trigger",
          description: "Required. `BuildTrigger` to update.",
          type: {
            type: "object",
            properties: {
              resourceName: {
      ...
      onEvent: async (input) => {
        ...
        if (request.trigger?.resource_name !== undefined) {
          const m = String(request.trigger?.resource_name).match(/^([^/]+)/);
...
export default updateBuildTrigger;
