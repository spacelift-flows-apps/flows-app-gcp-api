...
const instancesPointInTimeRestore: AppBlock = {
  name: "Instances - Point In Time Restore",
  ...
  inputs: {
    default: {
      config: {
        ...
        pointInTime: {
          ...
          type: {
            type: "string",
            description:
              "Required. The date and time to which you want to restore the instance. (Format: google-datetime)",
          },
          required: false,
        },
        targetInstance: {
      ...
      onEvent: async (input) => {
        ...
        let path = `v1/{+parent}:pointInTimeRestore`;
        ...
        if (input.event.inputConfig.datasource !== undefined)
          requestBody.datasource = input.event.inputConfig.datasource;
        if (input.event.inputConfig.pointInTime !== undefined)
          requestBody.pointInTime = input.event.inputConfig.pointInTime;
        if (input.event.inputConfig.targetInstance !== undefined)
          requestBody.targetInstance = input.event.inputConfig.targetInstance;
        if (input.event.inputConfig.privateNetwork !== undefined)
          requestBody.privateNetwork = input.event.inputConfig.privateNetwork;
        if (input.event.inputConfig.allocatedIpRange !== undefined)
          requestBody.allocatedIpRange =
            input.event.inputConfig.allocatedIpRange;
        if (input.event.inputConfig.preferredZone !== undefined)
          requestBody.preferredZone = input.event.inputConfig.preferredZone;
        if (input.event.inputConfig.preferredSecondaryZone !== undefined)
          requestBody.preferredSecondaryZone =
            input.event.inputConfig.preferredSecondaryZone;
...
export default instancesPointInTimeRestore;
